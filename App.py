...
# Only THREE is needed (we're avoiding submodules that import "three" internally)
...
# NOTE: This is a NORMAL triple-quoted string (NOT an f-string).
# That means we can safely use JS template literals (${...}) and braces {} without doubling.
HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <script type="module">
    ...
    async function setupThree(){
      const { WebGLRenderer, Scene, PerspectiveCamera, HemisphereLight, DirectionalLight,
              PlaneGeometry, TorusGeometry, SphereGeometry,
              MeshStandardMaterial, MeshBasicMaterial, Mesh, Color, TextureLoader, SRGBColorSpace } = THREE;
    ...
    async function recordWithFallback(duration, audioEl){
      ...
      try {
        for (const type of tryList){
          ...
          // FIX: wrap setTimeout in a Promise (setTimeout doesn't return a Promise)
          await Promise.race([
            done,
            new Promise((resolve)=> {
...
# Inject the data URL for THREE into the HTML (no f-string needed)
HTML = HTML.replace("{THREE_URL}", THREE_URL)
